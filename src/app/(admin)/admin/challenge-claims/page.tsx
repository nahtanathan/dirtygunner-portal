...
export default function AdminChallengeClaimsPage() {
  ...
  async function handleDelete(claim: ClaimItem) {
    const confirmed = window.confirm(
      `Delete this proof entry for "${claim.challengeTitle}" from ${claim.userLabel}? This will remove the claim record and attempt to delete the proof image from storage.`,
    );
    ...
    try {
      const res = await fetch(`/api/admin/challenge-claims/${claim.id}`, {
        method: "DELETE",
        cache: "no-store",
      });
        ...
        <section className="space-y-4">
          ...
          {loadingClaims ? (
            ...
            <div className="grid gap-4">
              {claims.map((claim) => {
                const isReviewing = reviewClaimId === claim.id;
                const isDeleting = deleteClaimId === claim.id;
                ...
                return (
                  ...
                  >
                    <div className="flex min-w-0 flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                      ...
                      <div className="grid gap-2 sm:grid-cols-2 xl:w-[240px] xl:grid-cols-1">
                        <button
                          type="button"
